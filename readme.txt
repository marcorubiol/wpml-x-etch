...
* UX: the JSON Loops tab now re-fetches the loop list and statuses in the background when activated, swapping the view if the list signature changed. Loops created in another tab or in the Etch builder appear in the panel without requiring a wp-admin reload. The REST `languages-status` endpoint now also returns `jsonLoops` so the JS bridge can keep its snapshot fresh.
